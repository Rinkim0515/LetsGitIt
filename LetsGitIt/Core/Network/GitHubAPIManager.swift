...
class GitHubAPIManager {
    ...
    // MARK: - User API
    func getCurrentUser(completion: @escaping (Result<GitHubUserDTO, GitHubAPIError>) -> Void) {
    ...
    // MARK: - Repository API
    func getUserRepositories(completion: @escaping (Result<[GitHubRepositoryDTO], GitHubAPIError>) -> Void) {
        ...
        session.dataTask(with: request) { data, response, error in
            DispatchQueue.main.async {
                ...
                do {
                    let repos = try JSONDecoder().decode([GitHubRepositoryDTO].self, from: data)
